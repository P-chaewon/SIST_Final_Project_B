...
<c:forEach items="${alarmVOs}" var="alarmVO">
	<div class="c">
		...
		<c:if test="${alarmVO.fromUserNum eq 1}">
			...
			<span class="text">${alarmVO.userVO.username}님이 회원님의 게시물을 좋아합니다.</span>
			<c:choose>
		...
		<c:if test="${alarmVO.fromUserNum eq 2}">
			<img class="profile" alt="프로필 사진" />
			<span class="text">${alarmVO.userVO.username}님이 댓글을 남겼습니다.</span>
			<c:choose>
				<c:when test="${alarmVO.diff eq 0}">
					<span class="text day">오늘</span>
				</c:when>
		...
		<c:if test="${alarmVO.fromUserNum eq 3}">
			<img class="profile" alt="프로필 사진" />
			<span class="text">${alarmVO.userVO.username}님이 회원님을 팔로우하기 시작했습니다.</span>
			<c:choose>
				<c:when test="${alarmVO.diff eq 0}">
					<span class="text day">오늘</span>
				</c:when>
